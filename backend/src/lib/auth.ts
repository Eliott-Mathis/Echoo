...
const auth = (prisma: PrismaClient = new PrismaClient()) =>
  betterAuth({
    ...
    plugins: [
      ...
      emailOTP({
        ...
        sendVerificationOTP: async ({ email, otp, type }: { email: string; otp: string; type: 'sign-in' | 'email-verification' | 'forget-password' }) => {
